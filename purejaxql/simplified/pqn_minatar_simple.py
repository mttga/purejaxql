...
def train(config, rng):
    ...
    # Create environment (MinAtar doesn't use FlattenObservationWrapper)
    ...
    # Create network (CNN for MinAtar)
    network = QNetwork(
    ...
    for update in range(int(num_updates)):
        ...
        # 4. LOGGING
        # Calculate env_frame (number of frames seen, MinAtar specific)
...
def main(config):
    ...
    if config.get("HYP_TUNE", False):
        ...
        print("Please use the original pqn_minatar.py for tuning.")
    else:
        single_run(config)
